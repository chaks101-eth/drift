...
export default function DesktopItemCard({ item, onClick, draggable, onDragStart, onDragOver, onDrop, onDragEnd, isDragging, isDragTarget, reaction, onReact, onStartPoll, poll, onVote, onApplyPoll, onClosePoll, isOwner }: Props) {
  ...
  return (
    <div
      role="button"
      tabIndex={0}
      aria-label={`${cat.label}: ${item.name}`}
      onClick={onClick}
      ...
      onDrop={onDrop}
      onDragEnd={onDragEnd}
      className={`group relative w-[230px] shrink-0 cursor-pointer overflow-hidden rounded-2xl border transition-all duration-400 focus:outline-none focus-visible:ring-2 focus-visible:ring-drift-gold/70 focus-visible:ring-offset-2 focus-visible:ring-offset-drift-bg
    ...
    >
      ...
      <div className="relative h-[140px] w-full overflow-hidden">
        ...
        <div className="absolute right-2 top-2 z-[3]" ref={menuRef}>
          <button
            ...
            className={`flex h-[28px] w-[28px] items-center justify-center rounded-full bg-black/65 backdrop-blur-md border border-[rgba(255,255,255,0.15)] text-white transition-opacity focus:outline-none focus:ring-2 focus:ring-drift-gold/60 ${
          ...
          {menuOpen && (
            <div className="absolute right-0 top-8 z-[10] min-w-[180px] rounded-xl border border-[rgba(255,255,255,0.08)] bg-[rgba(14,14,20,0.98)] backdrop-blur-xl shadow-[0_16px_48px_rgba(0,0,0,0.6)] overflow-hidden animate-[fadeUp_0.15s_ease]">
              <button
                ...
                className="flex w-full items-center gap-2.5 px-4 py-2.5 text-left text-[13px] text-drift-text hover:bg-drift-gold/[0.08] hover:text-drift-gold transition-colors"
              >
              ...
              <button
                ...
                className="flex w-full items-center gap-2.5 px-4 py-2.5 text-left text-[13px] text-drift-err hover:bg-drift-err/[0.12] transition-colors"
              >
                <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" strokeWidth="1.5">
        ...
        {/* Time badge */}
        {item.time && (
          <span className="absolute bottom-2.5 right-2.5 z-[2] rounded-md bg-black/60 backdrop-blur-md px-2 py-0.5 text-[11px] font-medium text-white">
            {item.time}
          </span>
        )}
      </div>
      ...
      <div className="p-4">
        {/* Category */}
        <div className={`mb-1.5 flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-[1.5px] ${cat.cls} opacity-90`}>
          {cat.icon}
          {cat.label}
        </div>

        {/* Name */}
        <div className="text-[15px] font-semibold leading-snug text-drift-text line-clamp-2 min-h-[40px]">{item.name}</div>

        {/* Meta */}
        <div className="mt-1 text-[12px] text-drift-text2 line-clamp-1">{item.detail}</div>

        {/* Price + Rating */}
        <div className="mt-3 flex items-center justify-between gap-2">
          <span className="text-[14px] font-semibold text-drift-text">{priceStr}</span>
          {rating && rating > 0 && (
            <span className="flex items-center gap-1 text-[12px] text-drift-text2">
          ...
          <div className="mt-3 pt-2.5 border-t border-white/[0.04] flex items-center gap-3">
            {onReact && (
              <button
                ...
                aria-label={reaction?.reacted ? 'Remove reaction' : 'Add reaction'}
                className={`flex items-center gap-1.5 text-[12px] transition-colors ${
                  reaction?.reacted ? 'text-drift-gold' : 'text-drift-text2 hover:text-drift-gold'
                }`}
              >
                <svg width="14" height="14" viewBox="0 0 24 24" fill={reaction?.reacted ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="1.5">
            ...
            {onStartPoll && !poll && (
              <button
                ...
                className="text-[12px] text-drift-text2 hover:text-drift-gold transition-colors"
              >
                Not sure?
              </button>
            )}
        ...
        {poll && poll.status === 'open' && onVote && (
          <div className="mt-2.5 pt-2.5 border-t border-white/[0.06] bg-white/[0.02] -mx-4 -mb-4 px-4 pb-4 rounded-b-2xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-2">
              <span className="text-[11px] font-semibold uppercase tracking-[1.5px] text-drift-gold">Group vote</span>
              {isOwner && onClosePoll && (
                <button type="button" onClick={onClosePoll} className="text-[11px] text-drift-text2 hover:text-drift-text transition-colors">Dismiss</button>
              )}
            </div>
              ...
              {poll.options.map((opt, i) => {
                ...
                return (
                  <button
                    ...
                    key={i}
                    onClick={() => onVote(i)}
                    className={`relative w-full rounded-lg border px-3 py-2 text-left text-[12px] overflow-hidden transition-all ${
                      voted ? 'border-drift-gold/40 text-drift-text' : 'border-white/[0.08] text-drift-text2 hover:border-white/20'
                    }`}
                  >
                    ...
                    <div className="relative flex justify-between">
                      ...
                      <span className="shrink-0 ml-2 text-drift-text2 tabular-nums">{opt.votes.length}</span>
                    </div>
                  </button>
                )
              })}
            ...
            {(() => {
              ...
              return (
                <button
                  type="button"
                  onClick={onApplyPoll}
                  className="mt-2 w-full rounded-lg bg-drift-gold/10 border border-drift-gold/25 py-2 text-[12px] font-semibold text-drift-gold hover:bg-drift-gold/20 transition-colors"
                >
